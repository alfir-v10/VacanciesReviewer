...
if __name__ == '__main__':
    text_list = ['data science', 'data scientist', 'data analyst', 'data engineer', 'python', 'Machine Learning', 'data', 'ML', 'DL', 'AI']
    for text in text_list:
        per_page = 100
        data = getVacancies(text, per_page=100)
        pages = data['pages']
        per_page = data['per_page']
        page = data['page']
        found = data['found']
        for p in range(0, pages+1):
            data = getVacancies(text, per_page=100, page=p)
            items = data['items']
            ...
            def check_key(session, table, id):
                if session.get(table, id):
                    return True
                return False
            ...
            for item in items:
                ...
                # if check_key(session, db.TypeVacancy, vacancy.vacancy_id)
